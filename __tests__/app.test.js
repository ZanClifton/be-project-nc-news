  ...
  describe("GET /api/articles", () => {
    ...
    describe("Unhappy Path", () => {
      test("404: returns an error if the path is not found", async () => {
        const res = await request(app).get("/api/ourtickles").expect(404);
    ...
    describe("Unhappy Path", () => {
      ...
      test("400: returns error if incorrect key names are used", async () => {
        ...
        const commentBody = {
          author: "butter_bridge",
          bawdy:
            "You should be looking everything up. I have very limited brain space and I want to save as much of it as possible for understanding. You're not supposed to know all these things; there's just too much.",
        };
    ...
    describe("Unhappy Path", () => {
      test("404: returns an error if user not found", async () => {
        ...
        expect(res.body).toMatchObject({
          msg: "unable to find articles by better_bodge!",
        });
      });
    });
  });
